...
#define repeat(v,c) (fmod(abs(v), c)-c/2.)
...
float2 toroidal (float2 p, float r) { return float2(length(p.xy)-r, atan2(p.y,p.x)); }

float3 lookAt (float3 eye, float3 at, float2 uv) {
	float3 forward = normalize(at - eye);
...
float cylinder (float2 p, float r) { return length(p)-r; }
float box (float3 p, float r) {	return length(max(abs(p) - r, float3(0,0,0))); }
float octaedre (float3 p, float r) { return dot(p, normalize(sign(p)))-r; }
...
void amod (inout float2 p, float count) {
